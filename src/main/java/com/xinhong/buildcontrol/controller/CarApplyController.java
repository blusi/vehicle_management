...
public class CarApplyController {
     ...
     * @return
     */
    @ApiOperation(value = "申请新增或修改——可用",httpMethod = "POST")
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "useInfo", value = "车辆使用信息", required = true,paramType = "query"),
            @ApiImplicitParam(name = "applyRoute", value = "行程", required = true,paramType = "query"),
            @ApiImplicitParam(name = "applyTime", value = "申请时间", required = true,paramType = "query"),
            @ApiImplicitParam(name = "applyStatus", value = "状态", required = true,paramType = "query"),
    })
    @RequestMapping(value="/save",method= RequestMethod.GET)//测试所用GET
    public Result save( CarApply carApply){//@RequestBoy
        try{
            if(carApply.getApplyId()!= null){
                carApply.setApplyModified(time.getTime1());
                carApply.setApplyOperator("初始值");
                carApplyService.updateById(carApply);
            }else {
                String s = IdUtil.simpleUUID();
                carApply.setApplyId(s);
                carApply.setApplyCreate(time.getTime1());
                carApply.setApplyOperator("初始值");
    ...
    public Result delete(@PathVariable("id") String id){
        try {
            CarApply pojo = new CarApply();
            pojo.setApplyId(id);
            pojo.setIsDelete(1);
            carApplyService.updateById(pojo);
            return result.success();
        } catch (Exception e) {
            e.printStackTrace();
            return result.fail(e.getMessage());
    ...
    {
        try {
            return result.success("成功",carApplyService.selectById(id));
        } catch (Exception e) {
            e.printStackTrace();
            return result.fail(e.getMessage());
        }
    ...
    public Result list(){
        try {
            EntityWrapper<CarApply> entityWrapper = new EntityWrapper<CarApply>();
            entityWrapper.and("is_delete=0");
            return result.success("成功",carApplyService.selectList(entityWrapper));
        } catch (Exception e) {
            e.printStackTrace();
            return result.fail(e.getMessage());
        }
    ...
    public Result listPage(@RequestParam(defaultValue = "1") Integer currentPage, @RequestParam(defaultValue = "10") Integer pageSize){
        try {
            ...
            List<CarApply> carApplies = carApplyService.selectList(null);
